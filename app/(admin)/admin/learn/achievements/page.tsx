...
import {
  DEFAULT_LEARN_ACHIEVEMENT_ICON,
  LEARN_ACHIEVEMENT_ICON_OPTIONS,
} from "@/lib/learn-achievement-icons";
...
export default function AdminLearnAchievementsPage() {
  ...
  const [deleteAchievement, { isLoading: isDeleting }] =
    useDeleteAdminLearnAchievementMutation();
  const [title, setTitle] = useState("Huy hiệu hoàn thành map");
  const [description, setDescription] = useState("Hoàn thành một map học được admin chỉ định.");
  const [iconUrl, setIconUrl] = useState<string>(DEFAULT_LEARN_ACHIEVEMENT_ICON);
  const [xpReward, setXpReward] = useState("100");
  const [showAllIcons, setShowAllIcons] = useState(false);
  const items = data?.items ?? [];
  const totalXpReward = items.reduce((sum, item) => sum + (item.xpReward ?? 0), 0);
  const iconCount = items.filter((item) => item.iconUrl).length;
  const visibleIconOptions = showAllIcons
    ? LEARN_ACHIEVEMENT_ICON_OPTIONS
    : LEARN_ACHIEVEMENT_ICON_OPTIONS.slice(0, ICON_PREVIEW_LIMIT);

  const createAchievementKey = (value: string) => {
    const slug = value
      .trim()
      .toLowerCase()
      .replace(/[^a-z0-9\s-]/g, "")
    ...
    return `${slug || "achievement"}_${Date.now().toString(36)}`;
  };
  ...
  const onCreate = async () => {
    if (!title.trim()) {
      notify({
        ...
        message: "Vui lòng nhập tên hiển thị cho huy hiệu.",
      });
      return;
    }
    ...
    try {
      await createAchievement({
        key: createAchievementKey(title),
        title: title.trim(),
      ...
      setTitle("");
      setDescription("");
      setIconUrl(DEFAULT_LEARN_ACHIEVEMENT_ICON);
      setXpReward("100");
      void refetch();
    } catch (createError) {
          ...
          <div className="flex flex-col gap-5 lg:flex-row lg:items-end lg:justify-between">
            <div>
              ...
              <p className="mt-3 max-w-3xl text-sm leading-7 text-slate-600 sm:text-base">
                Tạo huy hiệu thưởng XP và gắn làm phần thưởng hoàn thành cho từng map học.
                Trang này dành cho phần thưởng hệ thống, không phải nội dung bài học.
              </p>
            </div>
            <div className="rounded-3xl border border-amber-200 bg-amber-50 px-5 py-4 text-sm text-amber-900">
              <p className="font-semibold">Gợi ý sử dụng</p>
              <p className="mt-1 max-w-sm leading-6 text-amber-800">
                Sau khi tạo huy hiệu, mở trang chỉnh sửa map để chọn huy hiệu làm phần thưởng hoàn thành.
              </p>
            </div>
          </div>
        </section>
          ...
          <>
            <section className="grid gap-4 md:grid-cols-3">
              ...
              <Card className="border-slate-200 py-5">
                <CardContent className="flex items-start justify-between gap-4 pt-1">
                  <div>
                    <p className="text-sm text-slate-500">Đã chọn hình</p>
                    <p className="mt-2 text-3xl font-semibold tracking-tight text-slate-950">
                      {formatNumber(iconCount)}
                    </p>
                    <p className="mt-1 text-xs text-slate-500">
                      {formatNumber(items.length - iconCount)} huy hiệu chưa có hình riêng
                    </p>
                  </div>
              ...
              <Card className="border-slate-200 py-5">
                ...
                <CardContent className="space-y-4">
                  ...
                  <div className="space-y-3">
                    <Label>Hình huy hiệu</Label>
                    <div className="grid grid-cols-4 gap-3 sm:grid-cols-5">
                      {visibleIconOptions.map((option) => {
                        const selected = iconUrl === option.value;

                        return (
                          <button
                            key={option.value}
                            type="button"
                            onClick={() => setIconUrl(option.value)}
                            className={cn(
                              "group relative flex aspect-square items-center justify-center rounded-2xl border bg-white p-2 transition-all hover:-translate-y-0.5 hover:border-slate-300 hover:shadow-sm",
                              selected
                    ...
                    {LEARN_ACHIEVEMENT_ICON_OPTIONS.length > ICON_PREVIEW_LIMIT ? (
                      <Button
                        type="button"
                        variant="outline"
                        className="h-9 w-full rounded-xl border-slate-200 text-slate-700 hover:bg-slate-50"
                        ...
                        >
                          <div className="flex flex-col gap-4 lg:flex-row lg:items-start lg:justify-between">
                            <div className="flex min-w-0 gap-4">
                              <div className="flex h-16 w-16 shrink-0 items-center justify-center rounded-3xl border border-white bg-white shadow-sm">
                                {achievement.iconUrl ? (
                                  <img

...
type ExerciseReviewPageProps = {
  ...
  searchParams: Promise<{ answers?: string; score?: string; total?: string }>
}
...
export default async function ExerciseReviewPage({ params, searchParams }: ExerciseReviewPageProps) {
  ...
  const answers = parseAnswers(query.answers, exercise.questions.length)
    ...
    <ProtectedRoute>
      <main className="mx-auto w-full max-w-6xl px-6 py-10 lg:px-10">
        <section className="mb-8">
          <Link
            href={`/exercises/${exercise.id}/result?score=${query.score ?? ''}&total=${query.total ?? ''}&answers=${encodeURIComponent(query.answers ?? '')}`}

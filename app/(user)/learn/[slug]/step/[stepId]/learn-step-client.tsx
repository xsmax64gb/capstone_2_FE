...
export function LearnStepClient({ slug, step }: Props) {
  ...
  const turnProgress = Math.min(100, (learnerTurns / Math.max(minTurns, 1)) * 100)
  ...
  const replayAiLine = (content: string) => {
    if (!voice.synthesis || typeof window === 'undefined' || !content.trim()) return
    stopSpeaking()
  ...
  const startListening = () => {
    ...
    if (!RecognitionCtor) {
      setSpeechError(bi('Trình duyệt này chưa hỗ trợ speech-to-text.', 'This browser does not support speech-to-text yet.'))
      return
    }
    if (isListening) {
      stopListening('stop')
    ...
    recognition.onresult = (event) => {
      ...
      if (finalText.trim()) {
        setInput((current) => [current.trim(), finalText.trim()].filter(Boolean).join(' '))
      }
    ...
    try {
      ...
      setSpeechError(bi('Trình duyệt chưa thể bật micro. Hãy thử lại.', 'The browser could not start the microphone. Please try again.'))
    }
  }
  ...
  if (starting) {
    return (
      <div className="flex min-h-[60vh] flex-col items-center justify-center gap-3 text-slate-600">
        ...
        <p className="text-lg font-bold text-slate-900">{bi('Đang chuẩn bị buổi học nói…', 'Preparing the speaking lesson…')}</p>
      </div>
    )
  }
  ...
  if (!conversationId) {
    return (
      <div className="rounded-[28px] border border-red-200 bg-red-50 p-6 text-sm text-red-800 shadow-sm">
        {startError || bi('Không thể bắt đầu phiên học.', 'Unable to start the lesson session.')}
        <div className="mt-4">
    ...
    <div className="grid gap-6 xl:grid-cols-[320px_minmax(0,1fr)]">
      <aside className="space-y-4">
        <section className="overflow-hidden rounded-[32px] border border-slate-200 bg-gradient-to-br from-slate-950 via-slate-900 to-slate-800 p-6 text-white shadow-2xl">
          <div className="flex items-start justify-between gap-4">
            <div>
              <Badge className="rounded-full bg-white/10 px-3 py-1 text-[11px] font-semibold text-white">
                ...
                {step.type === 'boss' ? bi('Thử thách boss', 'Boss speaking check') : bi('Bài học nói', 'Voice lesson')}
              </Badge>
              <h2 className="mt-4 text-2xl font-extrabold tracking-tight">{teacherName}</h2>
              <p className="mt-2 text-sm leading-6 text-slate-300">
                {step.scenarioContext?.trim() || bi('Luyện nói tự nhiên, trả lời rõ ràng và phản hồi như một cuộc hội thoại thật.', 'Practice naturally, keep your answers clear, and respond like a real conversation.')}
              </p>
            </div>
          ...
          <div className="mt-5 grid grid-cols-2 gap-3">
            <div className="rounded-2xl border border-white/10 bg-white/10 p-3">
              <p className="text-[11px] uppercase tracking-[0.18em] text-slate-400">{bi('Chế độ', 'Mode')}</p>
              <p className="mt-1 text-sm font-semibold">{voice.recognition ? bi('Ưu tiên giọng nói', 'Voice first') : bi('Dùng văn bản thay thế', 'Text fallback')}</p>
            </div>
            <div className="rounded-2xl border border-white/10 bg-white/10 p-3">
              <p className="text-[11px] uppercase tracking-[0.18em] text-slate-400">{bi('Trạng thái', 'Status')}</p>
              <p className="mt-1 text-sm font-semibold">{isListening ? bi('Đang nghe…', 'Listening…') : isSpeaking ? bi('AI đang nói…', 'AI speaking…') : bi('Sẵn sàng', 'Ready')}</p>
            </div>
          </div>
        </section>
        ...
        <section className="rounded-[28px] border border-slate-200 bg-white p-5 shadow-sm">
          ...
          <h3 className="mt-3 text-lg font-bold tracking-tight text-slate-900">{step.title}</h3>
          {step.scenarioTitle && <p className="mt-1 text-sm text-slate-500">{step.scenarioTitle}</p>}
        ...
        <div className="grid gap-3 sm:grid-cols-2 xl:grid-cols-1">
          <Meter label={bi('Lượt của bạn', 'Your turns')} value={`${learnerTurns}/${minTurns}`} progress={turnProgress} tone="dark" />
          <Meter label={bi('XP nhận được', 'XP reward')} value={`+${step.xpReward ?? 0}`} progress={Math.min(100, ((step.xpReward ?? 0) / 40) * 100)} tone="emerald" />
        </div>
        ...
        {step.type === 'boss' && boss && !ended ? (
          <section className="rounded-[28px] border border-violet-200 bg-violet-50 p-5 shadow-sm">
            ...
            <div className="mt-4 space-y-4">
              <Meter label={bi('Máu boss', 'Boss HP')} value={`${boss.bossHPCurrent}/${boss.bossHPMax}`} progress={(boss.bossHPCurrent / Math.max(boss.bossHPMax, 1)) * 100} tone="violet" />
              <Meter label={bi('Máu của bạn', 'Your HP')} value={`${boss.playerHPCurrent}/${boss.playerHPMax}`} progress={(boss.playerHPCurrent / Math.max(boss.playerHPMax, 1)) * 100} tone="emerald" />
            </div>
            {boss.tasks?.length ? (
              <ul className="mt-4 space-y-2 text-sm text-violet-950">
                {boss.tasks.map((task) => (
                  <li key={task.id} className="flex items-start gap-2">
                    <span className={`mt-0.5 ${task.completed ? 'text-emerald-600' : 'text-violet-400'}`}>
                      {task.completed ? <CheckCircle2 className="h-4 w-4" /> : <RotateCcw className="h-4 w-4" />}
                    </span>
      ...
      <section className="space-y-4">
        <div className="rounded-[32px] border border-slate-200 bg-white shadow-xl">
          <div className="flex flex-col gap-4 border-b border-slate-200 px-5 py-5 lg:flex-row lg:items-center lg:justify-between">
            <div>
              ...
              <h3 className="mt-3 text-xl font-bold tracking-tight text-slate-900">{bi('Trò chuyện với giáo viên AI', 'Talk to your AI teacher')}</h3>
              <p className="mt-1 text-sm text-slate-500">{bi('Nói tự nhiên, theo dõi phiên âm, rồi gửi câu trả lời tốt nhất để nhận phản hồi.', 'Speak naturally, watch the transcript, then send your best answer for feedback.')}</p>
            </div>

            <div className="grid gap-3 sm:grid-cols-3">
              <div className="rounded-2xl bg-slate-50 px-4 py-3">
                <p className="text-[11px] font-semibold uppercase tracking-[0.18em] text-slate-400">{bi('Giáo viên', 'Teacher')}</p>
                <p className="mt-1 text-sm font-bold text-slate-900">{teacherName}</p>
              </div>
              <div className="rounded-2xl bg-slate-50 px-4 py-3">
                <p className="text-[11px] font-semibold uppercase tracking-[0.18em] text-slate-400">{bi('Mục tiêu', 'Goal')}</p>
                <p className="mt-1 text-sm font-bold text-slate-900">{bi(`${minTurns}+ lượt nói của bạn`, `${minTurns}+ learner turns`)}</p>
              </div>
              <div className="rounded-2xl bg-slate-50 px-4 py-3">
                <p className="text-[11px] font-semibold uppercase tracking-[0.18em] text-slate-400">{bi('Giọng nói', 'Voice')}</p>
                <p className="mt-1 text-sm font-bold text-slate-900">{voice.recognition ? bi('Mic sẵn sàng', 'Mic ready') : bi('Chỉ nhập chữ', 'Type only')}</p>
              </div>
            </div>
          </div>

          <div ref={transcriptRef} className="max-h-[52vh] space-y-4 overflow-y-auto bg-[radial-gradient(circle_at_top,_rgba(241,245,249,0.9),_transparent_48%)] px-5 py-5 md:px-6">
            {messages.map((message) => {
              const isAi = message.role === 'ai'

              return (
                <div key={message.id} className={`flex ${isAi ? 'justify-start' : 'justify-end'}`}>
                  <article className={`max-w-[92%] rounded-[26px] border px-4 py-4 shadow-sm md:max-w-[80%] ${isAi ? 'border-slate-200 bg-white text-slate-900' : 'border-slate-900 bg-slate-900 text-white'}`}>
                    <div className="mb-2 flex items-center gap-2">
                      <Badge variant="outline" className={isAi ? 'border-slate-200 bg-slate-50 text-slate-600' : 'border-white/20 bg-white/10 text-white'}>
                    ...
                    {message.grammarErrors?.length ? (
                      <div className={`mt-3 rounded-2xl border px-3 py-2 text-xs ${isAi ? 'border-amber-200 bg-amber-50 text-amber-900' : 'border-white/10 bg-white/10 text-slate-100'}`}>
                        <p className="font-semibold">{bi('Điểm cần sửa', 'Things to fix')}</p>
                        <ul className="mt-1 space-y-1">
                          {message.grammarErrors.slice(0, 2).map((item, index) => (
                            <li key={`${message.id}-grammar-${index}`}>
                              {item.message} {item.span ? `(${item.span})` : ''}
                            </li>
                          ))}
                        </ul>
                      </div>
        ...
        {ended ? (
          <div className="rounded-[32px] border border-slate-200 bg-white p-6 shadow-xl">
            <Badge className={`rounded-full px-3 py-1 ${ended.passed ? 'bg-emerald-600 text-white' : 'bg-slate-900 text-white'}`}>
              {ended.passed ? bi('Đạt', 'Passed') : bi('Xem lại và thử lại', 'Review and retry')}
            </Badge>
            <h3 className="mt-4 text-2xl font-extrabold tracking-tight text-slate-900">
              {ended.passed ? bi('Buổi học hoàn thành', 'Lesson completed') : bi('Phiên học đã kết thúc', 'Lesson session ended')}
            </h3>
            <p className="mt-3 max-w-2xl text-sm leading-6 text-slate-600">{ended.feedback}</p>
            ...
            <div className="flex flex-col gap-4 xl:flex-row xl:items-start">
              <div className="flex-1">
                ...
                <Textarea
                  ...
                  rows={4}
                  placeholder={bi('Nhập câu trả lời hoặc dùng micro để phiên âm hiện ở đây...', 'Type your answer or use the mic and let the transcript appear here...')}
                ...
                <p className="mt-3 text-sm text-slate-500">
                  {isListening
                    ? bi('Mic đang nghe. Hãy nói bằng tiếng Anh, phiên âm sẽ hiện ngay bên dưới.', 'The mic is listening. Speak in English and the transcript will appear below.')
                    : bi('Mẹo: nói 1-2 câu ngắn rồi chỉnh lại phiên âm nếu cần trước khi gửi.', 'Tip: say 1-2 short sentences, then edit the transcript if needed before sending.')}
                </p>
              ...
              <div className="grid gap-3 xl:w-[280px]">
                ...
                >
                  ...
                  {isListening ? bi('Dừng micro', 'Stop mic') : voice.recognition ? bi('Bắt đầu nói', 'Start speaking') : bi('Không có micro', 'No mic available')}
                </Button>

                <Button
                  type="button"
                  variant="outline"
                  onClick={() => latestAiMessage && replayAiLine(latestAiMessage.content)}
                ...
                >
                  {autoSpeak ? <Radio className="h-4 w-4" /> : <VolumeX className="h-4 w-4" />}
                  {autoSpeak ? bi('Tự đọc đang bật', 'Auto voice on') : bi('Tự đọc đang tắt', 'Auto voice off')}
                </Button>

                <Button
                  type="button"
                  onClick={() => void handleSend()}
                  disabled={sending || ![input.trim(), interimTranscript.trim()].filter(Boolean).join(' ').trim()}
              ...
              <div className="flex flex-wrap gap-2">
                <Badge variant="outline" className="rounded-full border-slate-200 bg-slate-50 px-3 py-1 text-slate-600">
                  ...
                  {learnerTurns < minTurns
                    ? bi(`Còn ${minTurns - learnerTurns} lượt để đủ mục tiêu`, `${minTurns - learnerTurns} more turns to reach the goal`)
                    : bi('Đã đủ số lượt tối thiểu', 'Minimum number of turns reached')}
                </Badge>
                <Badge variant="outline" className="rounded-full border-slate-200 bg-slate-50 px-3 py-1 text-slate-600">
                  <Brain className="h-3.5 w-3.5" />
                  {bi('Tập trung vào độ trôi chảy + rõ ràng', 'Focus on fluency + clarity')}
                </Badge>
              </div>

              <Button type="button" variant="outline" onClick={() => void handleEnd()} className="rounded-2xl border-slate-200">

...
import { BookOpen, Check, Lock, Map as MapIcon, Play, Sparkles } from 'lucide-react'
import {
  LearnPathOverlay,
...
import { useGetLearnMapsQuery, type LearnMapItem, type MapProgressStatus } from '@/lib/api/learnApi'

type NodeVisualStatus = 'completed' | 'current' | 'locked'

function toVisualStatus(status: MapProgressStatus | undefined): NodeVisualStatus {
...
export default function LearnMapsPage() {
  ...
  const primaryActiveIndex = items.findIndex((m) => m.progress?.status === 'active')
    ...
    <ProtectedRoute>
      <main className="mx-auto flex w-full max-w-[1440px] flex-1 overflow-hidden bg-gradient-to-b from-slate-50 via-white to-slate-50">
          ...
          <ScrollReveal>
            <header className="mx-auto max-w-2xl text-center">
              ...
              <p className="mt-3 text-sm leading-relaxed text-slate-600 md:text-base">
                Mỗi nút là một <strong>bản đồ học</strong>. Hoàn thành bài trong map và đánh bại boss để mở bản đồ
                tiếp theo. Tiếp tục để vào hội thoại AI.
              </p>
            </header>
          </ScrollReveal>

          <div ref={trackRef} className={`relative isolate mt-10 md:mt-14 ${LEARN_PATH_TRACK_CLASS}`}>
            <LearnPathOverlay containerRef={trackRef} anchorRefs={anchorRefs} anchorCount={items.length} />
            {isLoading && (
              <p className="text-center text-sm text-slate-500">Đang tải bản đồ…</p>
            )}
            {error && (
            ...
            {!isLoading && items.length === 0 && !error && (
              <p className="text-center text-sm text-slate-500">
                Chưa có bản đồ nào được xuất bản. Hãy chạy seed hoặc tạo map trong admin.
              </p>
            )}
            ...
            {items.map((map, index) => {
              ...
              const emphasizeFloat = visual === 'current' && index === primaryActiveIndex
        ...
        <aside className="hidden w-72 shrink-0 flex-col gap-6 border-l border-slate-200/80 bg-white/90 p-6 backdrop-blur xl:flex">
          <div>
            <h3 className="mb-4 text-xs font-bold uppercase tracking-wider text-slate-400">Gợi ý</h3>
            <div className="flex flex-col gap-3 text-sm text-slate-600">
              <div className="flex gap-3 rounded-xl border border-slate-100 bg-slate-50/80 p-3">
                ...
                <p>
                  Bản đồ <strong>khóa</strong> sẽ mở khi bạn hoàn thành map trước hoặc thắng boss.
                </p>
              </div>
              <div className="flex gap-3 rounded-xl border border-slate-100 bg-slate-50/80 p-3">
                <BookOpen className="mt-0.5 h-5 w-5 shrink-0 text-slate-700" />
                <p>
                  Vào từng map để xem <strong>các bước</strong> (lesson / boss) và bắt đầu hội thoại.
                </p>
              </div>
            </div>
          </div>
          ...
          <div className="mt-auto rounded-xl border border-slate-200 bg-slate-50 p-4">
            <p className="mb-2 text-xs font-semibold text-slate-500">Mục tiêu</p>
            ...
            <p className="mt-2 text-[11px] text-slate-500">
              {items.filter((m) => m.progress?.status === 'completed').length} / {items.length} bản đồ hoàn thành
            </p>
          </div>
        </aside>
      </main>
